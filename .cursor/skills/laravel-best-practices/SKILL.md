...
- Use Laravel Data DTOs (with TypeScript transformer), not Form Requests
- Per model: `ModelData` for response/front-end hydration and `ModelUpsertData` for validation
- Type-hint `ModelUpsertData` in `store`/`update` and return `ModelData` directly
...
- API-only backend: use `Route::apiResource()` for resources
- Controllers expose exactly 5 methods: `index`, `store`, `show`, `update`, `destroy`
- Methods under 10 lines — extract to actions/services
- Type-hint Laravel Data DTOs and return DTOs directly
